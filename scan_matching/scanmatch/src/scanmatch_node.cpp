...
const float CONVERGENCE_THRESHOLD = 1e-5;
...
public:
    ...
    void handleLaserScan(const sensor_msgs::msg::LaserScan::ConstSharedPtr msg)
    {
        ...
        while (count < MAX_ITER)
        {
            ...
            float delta = (curr_trans.x_disp - prev_trans.x_disp) * (curr_trans.x_disp - prev_trans.x_disp) +
                          (curr_trans.y_disp - prev_trans.y_disp) * (curr_trans.y_disp - prev_trans.y_disp) +
                          (curr_trans.theta_rot - prev_trans.theta_rot) * (curr_trans.theta_rot - prev_trans.theta_rot);
